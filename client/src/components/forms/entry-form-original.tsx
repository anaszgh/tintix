...
export function EntryForm({ onSuccess, editingEntry }: EntryFormProps) {
  ...
  // State for selected film type (global for all dimensions)
  ...
  // Effect to recalculate when dimensions change
  useEffect(() => {
      ...
      const jobEntryData = {
        ...
        dimensions: dimensions.map(dim => ({
          ...
          filmCost: selectedFilmId && films ? 
            (films.find(f => f.id === selectedFilmId)?.costPerSqft || 0) * ((dim.lengthInches * dim.widthInches) / 144) : 0,
